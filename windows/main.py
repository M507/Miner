...
def logic():
    ...
    while True:
        ...
        print(str(nextarrow_counter) + " nextarrow_counter")

        # Check if "iagree" is in the screenshot
        tmp_image = generic_check(current_screenshot, "iaccept")
        if tmp_image is not None and iaccept_counter < 10:
            go_to_image(tmp_image)
            iaccept_counter += 1
        # in case it got stuck
        elif iaccept_counter >= 10 and ((iaccept_counter % 10) == 0):
            pyautogui.press('enter')
            iaccept_counter += 1
        else:
            iaccept_counter += 1

        # Check if "iagrrebutton" is in the screenshot
        tmp_image = generic_check(current_screenshot, "iagreebutton")
        if tmp_image is not None:
            go_to_image(tmp_image)

        # Check if "ok" is in the screenshot
        tmp_image = generic_check(current_screenshot, "ok")
        if tmp_image is not None:
            go_to_image(tmp_image)

        # Check if "nextarrow" is in the screenshot
        tmp_image = generic_check(current_screenshot, "nextarrow")
        if tmp_image is not None:

            # The next condition solves the problem of having:
            # generic_check() where it finds an image but go_to_image() cannot find it
            # In a case like this, the counter will find the issue and
            # Will send all the list to make go_to_image() try all of them.
            if nextarrow_counter > 20 and nextarrow_counter <= 30:
                tmp_image_list = generic_check(current_screenshot, "nextarrow", True)
                for tmp_image in tmp_image_list:
                    go_to_image(tmp_image)
                nextarrow_counter += 1
            elif (check_if_it_exists(current_screenshot, "hasnotbeenacceptedc.PNG") and nextarrow_counter > 30):
        ...
        # Check if "install" is in the screenshot
        tmp_image = generic_check(current_screenshot, "install")
        if tmp_image is not None:
            go_to_image(tmp_image)
            continue

        # Check if "cancel" is in the screenshot
        tmp_image = generic_check(current_screenshot, "cancel")
        if tmp_image is not None:
            pass

        # Check if "arrowback" is in the screenshot
        tmp_image = generic_check(current_screenshot, "arrowback")
        if tmp_image is not None:
            pass

        # Check if "finish" is in the screenshot
        tmp_image = generic_check(current_screenshot, "finish")
        if tmp_image is not None:
            go_to_image(tmp_image)

        overall_counter += 1
        if overall_counter > 300:
            break
